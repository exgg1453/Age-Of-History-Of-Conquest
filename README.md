...
**Same network.** One device hosts and the others join. Hosts are found
automatically by UDP broadcast on the local network, and an address can also be
typed by hand if discovery is blocked. Every player picks their own country in
the lobby; two players cannot take the same one.

The host is authoritative. Clients send what they want to do, the host resolves
it, including every dice roll, and broadcasts the outcome so all devices apply
exactly the same result. After the AI has moved, the host sends a full state
snapshot, so a client that briefly desynchronises is corrected on the next turn
boundary rather than drifting.

Any country not taken by a player is run by the AI as usual, so three friends
...
Both live in the main menu and are remembered between sessions.

Difficulty is Easy, Normal, Hard or Impossible. It changes how much income the
AI collects, how favourable a fight it will accept, and how willing other
countries are to ally with you.

| Difficulty | AI income | AI combat | Alliance modifier |
|---|---|---|---|
| Easy | 75% | 85% | +25 |
| Normal | 100% | 100% | 0 |
| Hard | 135% | 115% | -20 |
| Impossible | 180% | 135% | -45 |

AI aggression is a separate dial from 1 to 100, adjusted in steps of ten. At 50
the world behaves normally. Raising it makes every country declare war on
thinner pretexts, run more wars at once, and attack at smaller margins.

| Aggression | War declared when stronger by | Chance per opportunity | Wars at once |
|---|---|---|---|
| 1 | 2.4x | 5% | 1 |
| 50 | 1.7x | 37% | 3 |
| 100 | 1.05x | 70% | 6 |

The two are independent, so a low difficulty world can still be a violent one.

### Alliances and relations

An alliance is not granted for asking. Every pair of countries has a relation
score, and an offer is accepted at 50 or above. Selecting a foreign province
shows its current score along with that country's government, religion and the
distance between your capitals, and a refusal names whichever factor cost the
most.

| Factor | Range | Notes |
|---|---|---|
| Base | +45 | every relation starts here |
| Relative power | -25 to +20 | equals attract; a minor power cannot buy a great power's friendship |
| Distance | -35 to +10 | measured capital to capital as a great circle; under 1500 km is a full bonus |
| Government | -28 to +16 | identical systems attract, opposed ideologies repel |
| Religion | -14 to +14 | same faith, then same family, then neither |
| Difficulty | -45 to +25 | see the table above |

The practical effect is that alliances are regional and ideological. A small
African republic cannot simply befriend Russia because it asked politely; the
power gap and the distance both bite. Two neighbours sharing a government and a
religion will pair off easily.

Government and religion live in `map/countries.json` and can be overridden per
country inside any scenario, so an alternate history mod can make 1914 Germany
a fascist state and watch the diplomatic map rearrange itself.
